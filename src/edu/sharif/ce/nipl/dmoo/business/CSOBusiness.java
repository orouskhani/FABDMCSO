...
public class CSOBusiness {
	...
	private List<Double> archiveTrueHVInChanges;
	private List<Double> archiveTrueAccInChanges;
	...
	public CSOBusiness(int nPop , int nVar1 , int nVar2 , double minVar1 , double maxVar1 , double minVar2 , double maxVar2 , int func , int cycle , int towet , int nt, int selectionAlgo, int reinitAlgo , int max_iter){
		...
		archiveTrueHVInChanges = new ArrayList<Double>();
		archiveTrueAccInChanges = new ArrayList<Double>();
		this.func = func;
	...
	private List<Integer> bordaCount(int iter) {
		...
		List<Integer> rankAggregatedIndices = functions.rankAggeragationAlgorithm(population , afterChangeCats , lastHVRValue , averageNormalchangeValue);
	...
	public void setCDC(int iter) {
		...
		System.out.println("Old SRD: " + SRD);
		...
		//Fuzzy adaptive
		double result[] = Coeffs.calSRDCDC(SRD, CDC, lastHVRValue, averageNormalchangeValue);
		SRD = result[0];
		CDC=  result[1];

		System.out.println("New CDC: " + CDC);
		System.out.println("New SRD: " + SRD);
	...
	public void calcTrueAcc(Front PFKnown){
		double[] referencedPoint = FrontUtils.getMaximumValues(PFKnown);
		
		Hypervolume hv = new Hypervolume();
		double hvKnown = hv.execute(PFKnown , referencedPoint);
		
		archiveTrueHVInChanges.add(hvKnown);
		archiveTrueAccInChanges.add(hvKnown / Collections.max(archiveTrueHVInChanges));
	...
	public Map<String , Double> printMetrics(int iteration) throws IOException {
		...
		double atac = averageList(archiveTrueAccInChanges);
		result.put("TrueAcc", atac);
	...
	public Map<String, List<Double>> getMeasures() {
		...
		result.put("TrueAcc", archiveTrueAccInChanges);

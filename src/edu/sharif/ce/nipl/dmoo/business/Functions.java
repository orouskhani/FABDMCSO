...
public class Functions {
	...
	public List<Integer> rankAggeragationAlgorithm(List<Cat> population1 , List<Cat> population2, double lastHVRValue, double averageNormalchangeValue) {
		List<Double> finalScore = new ArrayList<Double>();
		double prevAlpha = alpha;
		alpha  = Coeffs.calAlpha(lastHVRValue, averageNormalchangeValue, prevAlpha);
		...
		for (int i = 0 ; i < population1.size() ; i++) {
			...
			finalScore.add(alpha * (finalScore1) + (1 - alpha) * (finalScore2) + stabilityScore );// Modified Borda End*/
			//Normal Borda
